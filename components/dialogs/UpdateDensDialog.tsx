...
interface UpdateDensDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
}

export function UpdateDensDialog({ open, onOpenChange }: UpdateDensDialogProps) {
  ...
  const loadSedes = async (inicio: string, fin: string) => {
    try {
      ...
      const sedesData = await clientService.getSedesUpdateDensidades(inicio, fin)
  ...
  const handleSave = async () => {
    ...
    try {
      setSaving(true)
      await rateService.updateDensidades(startString, endString, selectedIds)
      toast({
        title: "Actualización realizada",
        description: "Las densidades se han actualizado correctamente",
        variant: "success",
      })
      onOpenChange(false)
    } catch (error: any) {
      toast({
        title: "Error",
        description: error?.message || "No se pudo actualizar densidades",
        variant: "destructive",
      })
    } finally {
      setSaving(false)
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium">Sedes con visitas M3 y densidad configurada</span>
              ...
              <div className="min-h-[48px]">
                ...
                {sedesOptions.length === 0 && (
                  <div className="text-sm text-muted-foreground mt-2">
                    No hay sedes con visitas M3 y densidad para el rango seleccionado.
                  </div>
                )}
              </div>
            )}
